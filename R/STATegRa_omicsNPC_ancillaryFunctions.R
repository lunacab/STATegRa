...
#this function computes association for continuous data
computeProjectedAssocContinuousData <- function(dataMatrix, design, outcomeName, useVoom = FALSE, mapping = NULL, ...){
  ...
  #if not mapping, then normal functions
  if(is.null(mapping)){
    return(computeAssocContinuousData(dataMatrix = dataMatrix, design = design, outcomeName = outcomeName, useVoom = useVoom, ...))
  }
  ...
  if(!surv){
    ...
    logPvalues <- (-2) * log(results$P.Value);
    names(logPvalues) <- rownames(results);
    
    #projecting miRNA statistics to gene statistics
    suppressMessages(projectedLogPvalues <- transferIndex(logPvalues , mapping));
    
    #return the values
    return(projectedLogPvalues)
    ...
    pvalues <- apply(dataMatrix, 1, modelFunction, design, outcomeId);
    
    #log transformed p-values
    logPvalues <- (-2) * log(pvalues);
    names(logPvalues) <- rownames(dataMatrix);
...
#this function computes association for count data
computeProjectedAssocCountData <- function(dataMatrix, design, outcomeName, mapping, ...){
  
  computeProjectedAssocContinuousData(dataMatrix, design, outcomeName, useVoom = TRUE, mapping, ...)
